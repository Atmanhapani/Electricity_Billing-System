...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("sending with: ",username+" ",password);

    try {
      const response = await fetch('http://localhost:7778/login/customer', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log("this is from server: "+token);
        sessionStorage.setItem("token",token);
        const a=sessionStorage.getItem("token");
        console.log("session token: "+a);
